...
contract TestMiniMeCloning {
  ...
  function testCanClone() {
    clone1 = MiniMeToken(token.createCloneToken("GNR2", 18, "GNR2", block.number, true));
  ...
  function testCanCloneAfterTransfer() {
    clone2 = MiniMeToken(token.createCloneToken("GNR2", 18, "GNR2", block.number, true));
  ...
  function testRecurringClones() {
    ...
    for (uint i = 0; i < 10; i++) {
      lastClone = MiniMeToken(lastClone.createCloneToken("GNRn", 18, "GNRn", block.number, true));
    }
